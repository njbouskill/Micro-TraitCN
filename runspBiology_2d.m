function [y, y_rst, time] = runspBiology(x0, dt, kend1, kend, ns,xx,yy)
%runBiology
%%
...
while(1)
    if(kk>0)
        ...
        if(k7 <= length(par0.no3_add_time))
           if(kk == par0.no3_add_time(k7))
              x(1,par0.id_no3x)=x(1,par0.id_no3x)+par0.no3_add_amount(k7);
             k7=k7+1;
           end
        end
        if(k8 <= length(par0.glucose_add_time))
           if(kk == par0.glucose_add_time(k8))
              x(1,par0.id_glucose)=x(1,par0.id_glucose)+par0.glucose_add_amount(k8);
             k8=k8+1;
           end
        end
        if(k9 <= length(par0.acetate_add_time))
           if(kk == par0.acetate_add_time(k9))
              x(1,par0.id_acetate)=x(1,par0.id_acetate)+par0.acetate_add_amount(k9);
             k9=k9+1;
           end
        end
    end
    ...
    o2_aq_frac    = (env.h2ovol(kk1,xx,yy)*bsc_o2)/(env.h2ovol(kk1,xx,yy)*bsc_o2...
        +env.airvol(kk1,xx,yy));                                                   %aqueous o2
    no_aq_frac    = (env.h2ovol(kk1,xx,yy)*bsc_no)/(env.h2ovol(kk1,xx,yy)*bsc_no...
        +env.airvol(kk1,xx,yy));                                                   %aqueous no
    %n2o_aq_frac   = (env.h2ovol(kk1)*bsc_n2o)/(env.h2ovol(kk1)*bsc_n2o+env.airvol(kk1));      %aqueous n2o
    nh4_2_nh3aq   = par0.ke_nh4_nh3*10.^(-env.pH(kk1,xx,yy));
    ...
    par0.nh4_fr_frac= nh4_2_nh3aq*env.h2ovol(kk1,xx,yy)*bsc_nh3...
        /(env.h2ovol(kk1,xx,yy)*bsc_nh3+env.airvol(kk1,xx,yy))/(nh4_2_nh3aq+1);            %nh3x:= nh3(g)+nh3(aq)+nh4(+)
